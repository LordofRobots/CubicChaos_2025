...
// Team+level colors (MUST match cube code exactly)
#define C_WHITE 0
#define C_BLUE_1 10
#define C_BLUE_2 11
...
struct __attribute__((packed)) CmdPkt {
  ...
  uint8_t msg_type;
  uint8_t sys_state;
  uint8_t color;     // 0/10/11/20/21 only
...
static const uint8_t STABLE_STREAK_REQ = 3;
...
struct Peer {
  ...
  uint8_t stableFace = 255;
  uint8_t stableFaceStreak = 0;
  ...
  // NEW: last protocol version seen from this cube (PairReq/StatusPkt)
  uint8_t proto = 0;
};
...
static void setAllWhite(bool resetLastFace) {
  for (int i = 0; i < MAX_CUBES; i++) {
    ...
    peers[i].stableFace = 255;
    peers[i].stableFaceStreak = 0;
    if (resetLastFace) peers[i].lastFace = 255;
  }
}
...
static void processFaceChange(int idx, uint8_t newFace) {
  if (gs.sysState != SYS_IN_GAME) return;

  Peer& p = peers[idx];

  if (p.stableFaceStreak < STABLE_STREAK_REQ) return;
  if (newFace == 255) return;

  if (p.lastFace == 255) {
    p.lastFace = newFace;
    return;
  }
  if (newFace == p.lastFace) return;
  p.lastFace = newFace;

  if (p.color == C_WHITE) {
    const bool giveBlue = gs.nextPullBlue;
    gs.nextPullBlue = !gs.nextPullBlue;
    p.color = makeTeamLevel_(giveBlue, 1);
    return;
  }

  if (isBlue_(p.color)) p.color = C_ORANGE_2;
  else if (isOrange_(p.color)) p.color = C_BLUE_2;
  else {
    const bool giveBlue = gs.nextPullBlue;
    gs.nextPullBlue = !gs.nextPullBlue;
    p.color = makeTeamLevel_(giveBlue, 1);
...
static void buildSnapshot() {
  ...
  for (int i = 0; i < MAX_CUBES; i++) {
    ...
    n = snprintf(out, rem,
                 ...
                 "\"face\":%u,\"stable\":%s,\"color\":%u,\"uptime_s\":%lu,\"ident\":%s,"
                 "\"proto\":%u}",
                 first ? "" : ",",
                 peers[i].id,
                 present ? "true" : "false",
                 health, (unsigned)good, (unsigned)loss20,
                 (unsigned)peers[i].face,
                 peers[i].stable ? "true" : "false",
...
/* ---- UPTIME COLOR INDICATION (BRIGHT / HIGH VISIBILITY) ---- */
.cell.uptime.ok{
...
function apply(j){
  ...
  // Keep the previous "standby/reset shows full time" behavior
  ...
  (j.cubes||[]).forEach((c,idx)=>{
    ...
    d.innerHTML = `
      ...
      <div class="kv">
        <div class="cell"><div class="k">FACE</div><div class="v">${n0(c.face)}</div></div>
        <div class="cell"><div class="k">STABLE</div><div class="v">${str01(c.stable)}</div></div>
        <div class="cell">
        ...
        <div class="cell"><div class="k">GOOD</div><div class="v">${n0(c.good)}/20</div></div>
        <div class="cell"><div class="k">IDENT</div><div class="v">${identOn ? 'on' : 'off'}</div></div>
      </div>
    `;
    g.appendChild(d);
  });
  ...
  <div class="grid">
    ...
    <div class="card">
      ...
      <div class="small">
        ...
        <ul style="margin:6px 0 0 18px;padding:0;color:var(--muted)">
          ...
          <li><span class="mono">/api/wifi</span> provides WiFi technical/client fields (RSSI/SSID/BSSID/etc).</li>
        </ul>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="small" style="margin-top:10px">
        Connected cubes is computed by the firmware (present within <span class="mono">PRESENT_MS</span>) and exposed as <span class="mono">count</span>.
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="small" style="margin-top:10px">
        DHCP lease seconds may be <span class="mono">—</span> depending on core/netif; other fields are live.
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="small" style="margin-top:10px">
        Current firmware exposes: <span class="mono">Channel</span> (from /api/diag) and per-cube <span class="mono">good/loss</span> derived from slot results (in /api/state).
        Additional ESPNOW counters require firmware instrumentation.
      </div>
    ...
    <div class="card" style="grid-column:1 / -1;">
      ...
      <div class="pills">
        ...
        <span class="pill"><span class="mono">HEALTH</span> <b class="mono">worst(comm, uptime)</b></span>
      </div>
      ...
      <div class="tableWrap">
        <table>
          ...
          <tbody id="rows">
            <tr><td colspan="12" class="small">loading...</td></tr>
          </tbody>
        </table>
      </div>
...
// comm health semantics identical to your server-side naming:
...
function renderWifiFields(){
  ...
  // RSSI severity is informational; only warn if very low.
...
function renderCubes(){
  ...
  if(!cubes.length){
    tb.innerHTML = `<tr><td colspan="12" class="small">no cubes</td></tr>`;
    return;
  }
  ...
  cubes.forEach((c, idx)=>{
    const present = !!c.present;
    const good = n0(c.good);
    const loss = (typeof c.loss !== 'undefined') ? n0(c.loss) : Math.max(0, 20-good);
    const proto = (typeof c.proto !== 'undefined') ? n0(c.proto) : '—';
    const sev = combinedHealth(good, c.uptime_s);

    html += `
      <tr>
        <td>${idx}</td>
        <td class="mono">${c.id || ('Cube '+(idx+1))}</td>
        <td>${present ? '1' : '0'}</td>
        <td><span class="${badgeClass(sev)}">${sev.toUpperCase()}</span></td>
        <td>${good}/20</td>
        <td>${loss}/20</td>
        <td>${proto}</td>
        <td>${n0(c.face)}</td>
        <td>${boolTxt(c.stable)}</td>
...
function renderAges(){
  const now = Date.now();
  const snapAge = lastSnapTs ? Math.max(0, Math.round((now-lastSnapTs)/1000)) : 999;
  const diagAge = lastDiagTs ? Math.max(0, Math.round((now-lastDiagTs)/1000)) : 999;
  const wifiAge = lastWifiTs ? Math.max(0, Math.round((now-lastWifiTs)/1000)) : 999;
  const sseAgeMs = lastSseTs ? (now-lastSseTs) : 1e9;

  snapAgeEl = document.getElementById('snapAge');
  diagAgeEl = document.getElementById('diagAge');
  wifiAgeEl = document.getElementById('wifiAge');

  snapAgeEl.textContent = `${snapAge}s`;
  diagAgeEl.textContent = `${diagAge}s`;
  wifiAgeEl.textContent = `${wifiAge}s`;

  pillSet('pSnap', `${snapAge}s`, (snapAge<=2)?'green':(snapAge<=5)?'yellow':'red');
  pillSet('pDiag', `${diagAge}s`, (diagAge<=2)?'green':(diagAge<=5)?'yellow':'red');
  pillSet('pWifiAge', `${wifiAge}s`, (wifiAge<=2)?'green':(wifiAge<=5)?'yellow':'red');

  let sseState='OFF', sseSev='yellow';
  if(sseOk){
    const stalled = (sseAgeMs > 3500);
    sseState = stalled ? 'STALLED' : 'ON';
    sseSev = stalled ? 'yellow' : 'green';
  }
  sse.textContent = sseState;
  pillSet('pSse', sseState, sseSev);
}
...
function forceFetch(){ fetchState(); fetchDiag(); fetchWifi(); }
...
function watchdog(){
  ...
  // If SSE stalls, restart.
  if (es && (Date.now() - (lastSseTs||0)) > 4000){
  ...
  if(autoOn){
    if((Date.now() - (lastSnapTs||0)) > 1500) fetchState();
    if((Date.now() - (lastDiagTs||0)) > 1500) fetchDiag();
    if((Date.now() - (lastWifiTs||0)) > 1500) fetchWifi();
  }
}
...
/* ===================== Timeline control ===================== */
static uint32_t g_endStartMs = 0;  // for diag LED stop timing (SYS_END_GAME entry)

/* ===================== WIFI DIAG METRICS (NEW) ===================== */
static uint32_t g_wifiReconnects = 0;
static uint32_t g_lastWiFiLossAtMs = 0;  // millis() timestamp when loss detected (0 = never)

static inline void ipToStr_(const IPAddress& ip, char out[16]) {
...
static void buildWifiJson_(String& s) {
  ...
  const bool connected = (WiFi.status() == WL_CONNECTED) && ipIsValid_(WiFi.localIP());
  ...
  IPAddress nmip = WiFi.subnetMask();
  IPAddress dns0 = WiFi.dnsIP(0);

  char gw[16], nm[16], dns[16];
  ipToStr_(gwip, gw);
  ipToStr_(nmip, nm);
  ipToStr_(dns0, dns);
  ...
  // LAST_WIFI_LOSS_MS is "age since last loss" in ms (or UINT32_MAX if never)
  uint32_t lastLossAge = 0xFFFFFFFFu;
  if (g_lastWiFiLossAtMs != 0) lastLossAge = (uint32_t)(millis() - g_lastWiFiLossAtMs);

  // DHCP lease seconds is not reliably exposed across cores; return null.
  s += "{";
  s += "\"status\":\"";
...
static void enterEndGame() {
  ...
  gs.matchRunning = false;
  gs.endSolidUntilMs = millis() + 2000;  // solid red then breathing
  g_endStartMs = millis();
}
...
static void enterReset() {
  ...
  gs.matchRunning = false;
  gs.resetLedUntilMs = millis() + 1000;  // solid purple then normal
}
...
// Sequences (global, referenced by sound system)
static const ToneStep TIMEGATE_SEQ[] = {
  { 1400, 80, 30 },
  { 2200, 80, 30 },
  { 3200, 90, 0 },
};

static const ToneStep RESET_SEQ[] = {
  { 2400, 50, 20 },
  { 700, 100, 100 },
};
...
public:
  enum class EvType : uint8_t { None = 0,
                                Beep = 1,
                                Seq = 2 };
  ...
  void begin(uint8_t pin, uint8_t ledcChannel = 0) {
    _pin = pin;
    _ch = ledcChannel;
  ...
  void playStartupBlocking() {
    ...
    for (uint8_t i = 0; i < (uint8_t)(sizeof(startup) / sizeof(startup[0])); i++) {
      if (startup[i].hz && startup[i].ms) {
        startTone_(startup[i].hz);
        delay(startup[i].ms);
        stopTone_();
      } else {
        stopTone_();
      ...
      if (startup[i].gap_ms) delay(startup[i].gap_ms);
    }
    stopTone_();
  }

  void tickState(uint8_t currentState) {
    if (currentState == _lastState) return;
    _lastState = currentState;
    enqueueForState_(currentState);
  }

  void tick() {
    const uint32_t now = millis();

    if (_activeType == EvType::None) {
      if (dequeue_(_active)) startActive_(now);
      return;
    }

    if (_activeType == EvType::Beep) {
      if ((int32_t)(now - _beepUntilMs) >= 0) {
        hardStop_();
        if (dequeue_(_active)) startActive_(now);
      }
      return;
    ...
    if (_activeType == EvType::Seq) {
      ...
      if (_inGap) {
        if ((int32_t)(now - _gapUntilMs) < 0) return;
        _inGap = false;
        _seqIdx++;
        if (_seqIdx >= _seqLen) {
          hardStop_();
        ...
        playSeqStep_(now, _seqIdx);
        return;
      }

      if ((int32_t)(now - _toneUntilMs) < 0) return;

      stopTone_();

      const ToneStep& s = _seq[_seqIdx];
      if (s.gap_ms) {
        _inGap = true;
        _gapUntilMs = now + (uint32_t)s.gap_ms;
        return;
      }
...
private:
  ...
  static constexpr uint8_t QN = 10;
  Event _q[QN]{};
  uint8_t _qh = 0, _qt = 0, _qc = 0;

  Event _active{};
  EvType _activeType = EvType::None;

  uint32_t _beepUntilMs = 0;

  const ToneStep* _seq = nullptr;
  uint8_t _seqLen = 0;
  uint8_t _seqIdx = 0;
  bool _inGap = false;
  uint32_t _toneUntilMs = 0;
  uint32_t _gapUntilMs = 0;

  uint8_t _lastState = 255;

  static constexpr uint16_t DUTY_ON = 256;  // 10-bit, ~25%
  static constexpr uint16_t DUTY_OFF = 0;

  void startTone_(uint16_t hz) {
    if (hz == 0) {
      stopTone_();
      return;
    }
#if defined(ESP_ARDUINO_VERSION_MAJOR) && (ESP_ARDUINO_VERSION_MAJOR >= 3)
    ledcWriteTone(_pin, (double)hz);
    ledcWrite(_pin, DUTY_ON);
#else
    ledcWriteTone(_ch, (double)hz);
    ledcWrite(_ch, DUTY_ON);
#endif
  }

  void stopTone_() {
#if defined(ESP_ARDUINO_VERSION_MAJOR) && (ESP_ARDUINO_VERSION_MAJOR >= 3)
    ledcWrite(_pin, DUTY_OFF);
    ledcWriteTone(_pin, 0);
#else
    ledcWrite(_ch, DUTY_OFF);
    ledcWriteTone(_ch, 0);
#endif
  }
  ...
  bool dequeue_(Event& out) {
    if (_qc == 0) return false;
    out = _q[_qh];
    _qh = (uint8_t)((_qh + 1) % QN);
    _qc--;
    return true;
  }

  void enqueue_(const Event& e) {
    if (_qc >= QN) return;
    _q[_qt] = e;
    _qt = (uint8_t)((_qt + 1) % QN);
    _qc++;
  }

  void enqueueForState_(uint8_t s) {
    switch (s) {
      case SYS_GAME_START:
        {
          Event e{};
      ...
      case SYS_TIME_GATE:
        {
          Event e{};
          e.type = EvType::Seq;
          e.u.seq = { TIMEGATE_SEQ, (uint8_t)(sizeof(TIMEGATE_SEQ) / sizeof(TIMEGATE_SEQ[0])) };
          enqueue_(e);
        }
        break;
      ...
      case SYS_RESET:
        {
          Event e{};
          e.type = EvType::Seq;
          e.u.seq = { RESET_SEQ, (uint8_t)(sizeof(RESET_SEQ) / sizeof(RESET_SEQ[0])) };
          enqueue_(e);
        }
        break;
  ...
  void startActive_(uint32_t now) {
    _activeType = _active.type;

    if (_activeType == EvType::Beep) {
      const uint16_t hz = _active.u.beep.hz;
      const uint16_t ms = _active.u.beep.ms;
      if (!hz || !ms) {
        hardStop_();
        return;
      }
      startTone_(hz);
      _beepUntilMs = now + (uint32_t)ms;
      return;
    }

    if (_activeType == EvType::Seq) {
      _seq = _active.u.seq.seq;
      _seqLen = _active.u.seq.len;
      _seqIdx = 0;
      _inGap = false;
      if (!_seq || _seqLen == 0) {
        hardStop_();
  ...
  void playSeqStep_(uint32_t now, uint8_t idx) {
    const ToneStep& s = _seq[idx];
    if (s.hz && s.ms) {
      startTone_(s.hz);
      _toneUntilMs = now + (uint32_t)s.ms;
    } else {
      stopTone_();
      _toneUntilMs = now + (uint32_t)s.ms;
    }
...
private:
  ...
  void renderChase_(const CRGB& color, uint8_t trailDim, uint16_t speedMs, uint32_t now) {
...
static void commsTickFast_() {
  ...
  // If no active slot, see if we're allowed to start the next one yet.
  if (slotIdx < 0) {
  ...
  auto sendNormalCmdOnce_ = [&]() {
    ...
    sendCmdRaw_(slotIdx, 0, 0);
  };

  // If status arrived and was accepted by RX, close immediately (fast advance).
  if (p.hitThisSlot) {
    p.hitThisSlot = false;

    // Close slot as SUCCESS
    p.awaiting = false;
    ...
    // Next slot starts after a 10ms gap
    slotIdx = -1;
    nextSlotDueMs = now + COMM_NEXT_GAP_MS;
    return;
  }

  // Maintain prior behavior: send normal cmd shortly after poll,
  // even if status hasn’t arrived yet.
  if (p.awaiting && (uint32_t)(now - p.pollSentMs) >= CMD_SEND_AT_MS) {
    sendNormalCmdOnce_();
  }

  // Timeout: no valid response in 50ms => close slot as FAIL, advance after 10ms.
  if ((int32_t)(now - slotDeadlineMs) >= 0) {
    // Close slot as FAILURE
    p.awaiting = false;
  ...
  (void)slotStartMs;  // retained for debugging symmetry; not used in fast mode
}
...
static void onRecv(const esp_now_recv_info_t* info, const uint8_t* data, int len) {
  ...
  if (mt == MSG_PAIR_REQ && len == (int)sizeof(PairReq)) {
    ...
    if (idx < 0) {
      ...
      peers[idx].lastFace = 255;
      peers[idx].stableFace = 255;
      peers[idx].stableFaceStreak = 0;
    }

    // Record proto version from PairReq
  ...
  if (mt == MSG_STATUS && len == (int)sizeof(StatusPkt)) {
    ...
    // Must match the currently awaited poll seq and be within the slot window.
    ...
    // Record proto version from StatusPkt
    p.proto = st.proto_ver;

    p.lastSeenMs = now;
    p.face = st.face;
    p.stable = (st.stable20 != 0);
    p.uptime_s = st.uptime_s;

    if (p.stable) {
      if (p.stableFace == st.face) {
        if (p.stableFaceStreak < 255) p.stableFaceStreak++;
      } else {
        p.stableFace = st.face;
        p.stableFaceStreak = 1;
      }
    } else {
      p.stableFace = 255;
      p.stableFaceStreak = 0;
    }

    if (gs.sysState != SYS_END_GAME) processFaceChange(idx, p.stableFace);

    // Mark slot hit (consumed by commsTick50ms()).
    p.hitThisSlot = true;
    return;
  }
}
...
static void buildDiagText_(String& s) {
  ...
  s += "\n";
  s += "Channel: ";
  s += String((int)wifiChannel);
  s += "\n";
  s += "State: ";
  s += String(stateName((uint8_t)gs.sysState));
  s += "\n";
  s += "MatchRunning: ";
  ...
  for (int i = 0; i < MAX_CUBES; i++) {
    ...
    s += "  face=";
    s += String((unsigned)peers[i].face);
    s += "  stable=";
    s += (peers[i].stable ? "1" : "0");
    s += "  color=";
...
static void setupWeb() {
  ...
  // NEW: WiFi technical/client JSON (populates /diag fields)
...
void loop() {
  ...
  // Event-driven cube comms (runs continuously)
  commsTickFast_();
